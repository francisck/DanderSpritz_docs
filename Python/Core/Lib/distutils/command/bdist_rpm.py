...
# Embedded file name: bdist_rpm.py
...
class bdist_rpm(Command):
    ...
    boolean_options = [
     ...
     'no-autoreq', 'quiet']
    negative_opt = {'no-keep-temp': 'keep-temp','no-rpm-opt-flags': 'use-rpm-opt-flags',
       'rpm2-mode': 'rpm3-mode'
       }
    ...
    def finalize_package_data(self):
        ...
        self.ensure_string('vendor', '%s <%s>' % (self.distribution.get_contact(),
    ...
    def run(self):
        ...
        self.execute(write_file, (
         spec_path,
         self._make_spec_file()), "writing '%s'" % spec_path)
        if self.spec_only:
            return
        else:
