...
class EventLogFilter(dsz.data.Task):
    ...
    def _LoadData(self):
        ...
        try:
            for x in dsz.cmd.data.Get('Record', dsz.TYPE_OBJECT):
                self.Record.append(EventLogFilter.Record(x))
        ...
        def __init__(self, obj):
            ...
            try:
                for x in dsz.cmd.data.ObjectGet(obj, 'String', dsz.TYPE_OBJECT):
                    self.String.append(EventLogFilter.Record.String(x))
...
dsz.data.RegisterCommand('EventLogFilter', EventLogFilter)
