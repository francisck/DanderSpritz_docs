...
# Embedded file name: modulefinder.py
    ...
    def scan_opcodes_25(self, co, unpack=struct.unpack):
        ...
        while code:
            ...
            if code[:9:3] == LOAD_LOAD_AND_IMPORT:
                ...
                if level == -1:
                    yield (
                     'import', (consts[oparg_2], names[oparg_3]))
                elif level == 0:
                    yield (
                     'absolute_import', (consts[oparg_2], names[oparg_3]))
                else:
                    yield (
                     'relative_import', (level, consts[oparg_2], names[oparg_3]))
                code = code[9:]
                continue
            if c >= HAVE_ARGUMENT:
                code = code[3:]
