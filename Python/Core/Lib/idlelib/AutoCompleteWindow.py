...
# Embedded file name: AutoCompleteWindow.py
"""
...
class AutoCompleteWindow:
    ...
    def keypress_event(self, event):
        ...
        if keysym != 'Tab':
            self.lastkey_was_tab = False
        if (len(keysym) == 1 or keysym in ('underscore', 'BackSpace') or self.mode == COMPLETE_FILES and keysym in ('period',
        ...
        if keysym == 'Return':
            self.hide_window()
            return
        if self.mode == COMPLETE_ATTRIBUTES and keysym in ('period', 'space', 'parenleft',
                                                           'parenright', 'bracketleft',
                                                           'bracketright') or self.mode == COMPLETE_FILES and keysym in ('slash',
                                                                                                                         'backslash',
                                                                                                                         'quotedbl',
        ...
        if keysym == 'Tab' and not state:
            ...
            if any((s in keysym for s in ('Shift', 'Control', 'Alt', 'Meta', 'Command',
                                          'Option'))):
                return
            self.hide_window()
            return
