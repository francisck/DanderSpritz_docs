...
# Embedded file name: Mcl_Cmd_ProcessModify_DataHandler.py
...
def DataHandlerMain(namespace, InputFilename, OutputFilename):
    ...
    mcl.imports.ImportNamesWithNamespace(namespace, 'mca.process.cmd.processmodify', globals())
    input = mcl.data.Input.GetInput(InputFilename)
    output = mcl.data.Output.StartOutput(OutputFilename, input)
    output.Start('ProcessModify', 'processmodify', [])
    msg = mcl.object.Message.DemarshalMessage(input.GetData())
    if input.GetStatus() != mcl.status.MCL_SUCCESS:
        errorMsg = msg.FindMessage(mcl.object.Message.MSG_KEY_RESULT_ERROR)
        moduleError = errorMsg.FindU32(mcl.object.Message.MSG_KEY_RESULT_ERROR_MODULE)
        osError = errorMsg.FindU32(mcl.object.Message.MSG_KEY_RESULT_ERROR_OS)
        if moduleError == ERR_MODIFICATION_FAILED:
